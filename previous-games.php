...
<body>
  ...
  if(isset($_GET["user"]) && isset($user) && $user->isLoggedIn() && $user->data()->id == 1) {
    ...
    if($db->error())
        file_put_contents ("error_log", "previous-games line 39: ".$db->errorString(), FILE_APPEND);
    ...
    foreach($results as $row) {
        ...
        if($row->user_id_black == $user->data()->id) {
            ...
            if($db->error())
                file_put_contents ("error_log", "previous-games line 53: ".$db->errorString(), FILE_APPEND);
        ...
        if($row->user_id_white == $user->data()->id) {
            ...
            if($db->error())
                file_put_contents ("error_log", "previous-games line 62: ".$db->errorString(), FILE_APPEND);
            echo $query->first()->username;
        }
