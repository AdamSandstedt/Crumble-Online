...
async function minmax(gameState, gameRules, evaluate, plies, alpha = Number.NEGATIVE_INFINITY, beta = Number.POSITIVE_INFINITY, statisticsHook) {
    ...
    var promise = new Promise(function(resolve, reject) {
      setTimeout(async function() {
        ...
        if ( (vForTerminal!=null) || (pliesRemaining===0)) {
          ...
          resolve(new EvaluationAndMove(null, v2*(maximizing?1:-1)));
        } else {
          ...
          if (maximizing) {
            ...
            for (let i = 0; i < NUM_OF_MOVES ; i++) {
              ...
              const nextStateEval = await _minmax(nextState, pliesRemaining-1, Math.max(v, alpha), beta, !maximizing);
              if (nextStateEval!=null) {
                if (nextStateEval.evaluation > v) {
                  ...
                  resolve(new EvaluationAndMove(moves[i], nextStateEval.evaluation));
            ...
            resolve(new EvaluationAndMove(bestMove!==null?bestMove:moves[0], v)); // if all moves are equally bad, return the first one
          } else {
            var v = Number.POSITIVE_INFINITY;
            var bestMove = null;
            for (let i = 0; i < NUM_OF_MOVES ; i++) {
              const nextState = gameRules.nextState(gameState, moves[i]);
              const nextStateEval = await _minmax(nextState, pliesRemaining-1, alpha, Math.min(v,beta), !maximizing);
              if (nextStateEval!=null) {
  ...
  return {
    ...
    const evalAndMove = await _minmax(gameState, plies, alpha, beta, true); // in the min-max algorithm the player who is to make the move is the maximizing player
