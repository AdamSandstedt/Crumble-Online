...
function setupGraphics(cgame, canvas, icanvas, autoResize, gameId, table) {
      ...
      if(gameId) {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                xmlhttp.open("GET", "/assets/php/get_times.php?game="+gameId, true);
                xmlhttp.send();
                this.onreadystatechange = function() {
                    if (this.readyState == 4 && this.status == 200) {
                        ...
                        if((cgame.history.length == 0 ? "" : cgame.history[cgame.history.length-1]) == this.responseText) {
                            ...
                            if(cgame.time && cgame.timeb && cgame.timew) {
                                if(cgame.turn == "b") {
                                    cgame.timeb -= time - cg.time;
                                } else {
                                    cgame.timew -= time - cg.time;
                                }
